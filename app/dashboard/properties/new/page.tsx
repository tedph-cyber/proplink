...
export default function NewPropertyPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const valid = files.filter(f => {
      ...
      if (f.size > 25 * 1024 * 1024) { setError('Files must be under 25MB'); return false }
      return true
    })
          ...
          {step === 1 ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className={styles.formFields}>
                {/* Drop zone */}
                <div>
                  <label className={styles.formLabel}>Photos & Videos <span className={styles.formLabelHint}>max 10 files, 25MB each</span></label>
                  <label className={styles.dropZone}>
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className={styles.dropZoneSub}>JPG, PNG, MP4 up to 25MB</span>
                    </div>
